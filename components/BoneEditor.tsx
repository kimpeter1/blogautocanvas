...
import { suggestBonePrompt } from '../services/geminiService';
...
export const BoneEditor: React.FC<BoneEditorProps> = ({ isOpen, onClose, image, onApply }) => {
    ...
    const handleSuggest = async () => {
        ...
        setError(null);
        try {
            const suggestion = await suggestBonePrompt(image);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <div className="neu-card w-full max-w-2xl flex flex-col gap-4 p-6" onClick={(e) => e.stopPropagation()}>
                <header className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-slate-900">뼈 구조 시각화</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="neu-card-inset flex items-center justify-center p-1 overflow-hidden">
                        <img src={imageUrl} alt="Source for Bone Visualization" className="max-w-full max-h-80 object-contain rounded-lg" />
                    </div>
                    <div className="flex flex-col gap-4">
                        <p className="text-sm text-slate-700">
                           이미지 위에 그리고 싶은 뼈의 구조를 설명해주세요. (예: 손목의 뼈를 그려주세요)
                        </p>
                        <textarea
                            placeholder="시각화 요청..."
                            rows={5}
